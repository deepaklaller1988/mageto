...
class Magestore_Inventoryplus_Adminhtml_AdjuststockController extends Mage_Adminhtml_Controller_Action {
            ...
            try {
                ...
                $adjuststockHelper = Mage::helper('inventoryplus/adjuststock');
                 if (count($dataFile)){
                    ...
                    echo Mage::helper('inventoryplus')->__("File is empty");
                    return;
                }
                if(count($adjuststockProducts) > 0){   
                    $adjuststockHelper->importProduct($adjuststockProducts);
                } else {         
                    echo Mage::helper('inventoryplus')->__("Wrong content's structure");
                    return;
                }
                ...
                if($this->getRequest()->getParam('id')){
                    ...
                    if ($this->getRequest()->getParam('cancel')){
                        ...
                        Mage::getSingleton('adminhtml/session')->addSuccess(
                            Mage::helper('inventoryplus')->__('The stock adjustment has been successfully cancelled.')
                        );
                        $this->_redirect('*/*/');
                        return;
                    }
    ...
    {
        $fileName   = 'inventory_adjust_product_log.csv';
        $content    = $this->getLayout()->createBlock('inventoryplus/adminhtml_adjuststock_edit_tab_products')
            ->getCsv();
    ...
    {
        $fileName   = 'inventory_adjust_product_log.xml';
        $content    = $this->getLayout()->createBlock('inventoryplus/adminhtml_adjuststock_edit_tab_products')
            ->getXml();
